...
public class bonfireBehaviour : MonoBehaviour
{
    ...
    private void Update()
    {
        if ((inputManager.getKeyDown(inputEnum.Interact.ToString()) && _playerIsOn && !UIController.getIsPaused() && !UIController.getIsLevelingUp() && !UIController.getIsAdquiringSkills() && !UIController.getIsLevelingUpWeapon() && !UIController.getIsInInventory()))
        {
            useBonfireUI();
        }
    }
